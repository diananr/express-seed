...
const signup = async(user) => {
    try {
        if (!user.email || !user.password)
            return { error: "messages.GLOBAL.ERROR", msg: "messages.AUTH.INVALID_AUTH" };

        const emailRegistered = await findUserByEmail(user.email);
        if(emailRegistered.length != 0)
            return { error: "messages.GLOBAL.ERROR", msg: "messages.AUTH.EMAIL_ALREADY_EXIST" };
...
const login = async(body) => {
    try {
        ...
        if (userPassword != requestHashedPassword)
            return { error: "messages.GLOBAL.ERROR", msg: "messages.AUTH.INVALID_PASSWORD" };
